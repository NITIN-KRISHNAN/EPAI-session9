...
NUM_PROFILE = 10000
Profile = namedtuple('Profile', 'job, company, ssn, residence, current_location, blood_group, website, username, '
...
ProfileList.__doc__ = "ProfileList namedtuple that contains all the entries of profiles as a separate field whose" \
                      "name is _1, _2 etc"
...
def form_profile_named_tuple() -> ProfileList:
    ...
    profile_list = ProfileList(*profile_tuple_list)
    return profile_list


def calculate_largest_blood_type(profile_list) -> tuple:
    """
...
def calculate_mean_current_location(profile_list) -> tuple:
    """
    ...
    mean_current_location = sum_x/ len(profile_list), sum_y / len(profile_list)
    end_time = time.time()
    return mean_current_location, end_time - start_time


def calculate_oldest_person_age(profile_list) -> tuple:
    """
    ...
    """
    start_time = time.time()
    max_age = max((datetime.date.today() - profile.birthdate).days for profile in profile_list) / 365.25
    end_time = time.time()
    return max_age, end_time - start_time


def calculate_avg_age(profile_list) -> tuple:
    """
    ...
    """
    start_time = time.time()
    avg_age = sum((datetime.date.today() - profile.birthdate).days for profile in profile_list) /\
              len(profile_list) / 365.25
    end_time = time.time()
